...
if __name__ == '__main__':
    ...
    max_iteration = 1000
    n_particles = 100
    x_data, bicha_data, jiaocha_data = read_excel_file(file_name=file_name, sheet_name=sheet_name)
    ## 比差的组合回归
    ...
    print("##################### bi cha ##############################")
    print(bicha_error)
    save_data_as_csv(file_name=file_name, save_data=np.array(list(bicha_error.values())).reshape(5,-1), string='比差回归误差')
    print("####################### jiao cha ##############################")
    print(jiaocha_error)
    save_data_as_csv(file_name=file_name, save_data=np.array(list(jiaocha_error.values())).reshape(5, -1), string='角差回归误差')
